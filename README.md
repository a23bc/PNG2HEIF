...
**HEIC 编码失败**（`CGImageDestinationFinalize` 返回 false，本身不报原因）。现在：

- **两个候选目录依次重试**：容器临时目录 → `/tmp/png2heif`，一个失败就换下一个，
  两边都失败时把**两边的原始原因**一起写进失败列表
- 失败时**铺白底去掉 alpha 再编一次**（HEIF 里没有 alpha 通道，带 alpha 的 PNG 有时会失败）
- 失败原因带上尺寸、`alphaInfo` 与目录，下次不用再猜
